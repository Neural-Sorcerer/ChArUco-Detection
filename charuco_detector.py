...
class CharucoDetector:
    ...
    def generate_board_image(self,
        ...
        # Margin is a percentage of the board size
        margin = 0.1
        margin_size = int(margin * min(width_grid, height_grid))
        
        # Calculate image size
        width = width_grid + (2 * margin)
        height = height_grid + (2 * margin)
    ...
    def save_board_image(self,
        ...
        try:
            # Generate board image
            board_img = self.generate_board_image(pixels_per_square, margin, border_bits)
